...
class GetReportDataByVictimGenderTask extends ParentTask
{
    ...
    {
        $catalog = app(FindCatalogByCodeTask::class)->run('GEN');
        $title = 'Género de la víctima';

        $data = [];
        foreach ($catalog->items as $index => $catalog_item) {
            $rows = (clone $denunciations)->where('victim_gender', $catalog_item->name)->get();
            $data[] = [
