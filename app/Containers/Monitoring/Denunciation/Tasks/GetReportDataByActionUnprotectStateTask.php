...
class GetReportDataByActionUnprotectStateTask extends ParentTask
{
    ...
    {
        $catalog = app(FindCatalogByCodeTask::class)->run('APE');
        $title = 'Acciones de desprotección del estado';

        $data = [];
        foreach ($catalog->items as $index => $catalog_item) {
            $rows = (clone $denunciations)->where('action_unprotect_state', $catalog_item->name)->get();
            $data[] = [
