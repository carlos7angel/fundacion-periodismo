...
class GetReportDataByActionResponseStateTask extends ParentTask
{
    ...
    {
        $catalog = app(FindCatalogByCodeTask::class)->run('ARE');
        $title = 'Acciones/respuestas del estado';

        $data = [];
        foreach ($catalog->items as $index => $catalog_item) {
            $rows = (clone $denunciations)->where('action_response_state', $catalog_item->name)->get();
            $data[] = [
