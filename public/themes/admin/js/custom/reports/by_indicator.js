...
var KTReportsByIndicator = function () {
    ...
    var initChart = function () {
        ...
        element = document.getElementById("kt_report_by_indicator_chart");
        if (!element) { return; }
    ...
    var renderTable = function (data) {
        ...
        let table_container_name = '#kt_report_by_indicator_table';
    ...
    var renderCard = function(data, total) {
        ...
        const container = document.getElementById('kt_report_by_indicator_card');
        container.innerHTML = '';

        data.forEach(item => {
            let percentage = 0;
            if (total > 0) {
                percentage = ((item.total / total) * 100).toFixed(0);
            }
        ...
        const container_total = document.getElementById('kt_report_by_indicator_total');
    ...
    var selectDropdown = function () {
        ...
        const optionFormat = (item) => {
            ...
            if (item.id.includes("CAT_")) {
                template = '<span class="fs-6 fw-bolder">' + item.text + '</span>';
            }

            if (item.id.includes("ITEM_")) {
                template = '<span class="fs-7 ps-5"> - ' + item.text + '</span>';
            }
        ...
        $dropdown.select2({
            placeholder: $dropdown.data('placeholder') || 'Seleccione una opción',
    ...
    var reloadReport = function(start_value = null, end_value = null, category_value = null) {
        ...
        var container = document.getElementById("kt_report_by_indicator_chart")
        ...
        am5.net.load(url).then(function(result) {
            ...
            // RENDER TITLES
            renderTitle(data.data.title, data.data.category);
    ...
    var renderTitle = function (title, category) {
        $('.kt_report_by_indicator_title').html(title);

        let label_cat = category ? 'Tipo de Agresión: ' + category : '';
        $('.kt_report_by_indicator_category').html(label_cat);
    }
